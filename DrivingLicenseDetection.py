...
# plate_detecter.predict("C:/Users/puliv/OneDrive/Desktop/HelmetDetection/Models/model_weights.h5")
# plate_detecter.predict()
...
def detectBike():
    ...
    try:
        ...
        image, ops = detectObject(cnn_model, cnn_layer_names, image_height, image_width, image, label_colors,
...
def drawPred(classId, conf, left, top, right, bottom, frame, option):
    ...
    if label_name == 'Helmet' and conf > 0.50:
        ...
        if option == 0 and conf < 0.90:
            ...
            account_sid = "ACc379a30fe7338ea008c9d8221cd01be2"
            auth_token = "1f9256d7723a07846eb8e111f4c3faf9"
            client = Client(account_sid, auth_token)
            message = client.messages.create(
                body="you have committed a traffic violation on said date and time.please pay the 'X' amount fine within seven days to avoid any legal action.This is the notice against the vehicle number:" + str(labels_value[predict]),
                from_="+15855846956",
                to="+919942912786"
            )
            print(message.sid)
